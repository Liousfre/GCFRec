...
class TrainDataset(torch.utils.data.Dataset):
    def __init__(self, id2seq, max_len, parallel_ag=False, masked_lm=False):
        self.id2seq = id2seq
    ...
    def __getitem__(self, index):
        ...
        if self.masked_lm:
            ...
            return torch.LongTensor(seq_pad), torch.zeros(self.max_len, dtype=torch.long)
        hist = seq[:-1]
...
class Data_Train():
    def __init__(self, data_train, args):
        ...
        self.masked_lm = args.model == 'bert4rec'
        if self.split:
    ...
    def get_pytorch_dataloaders(self):
        dataset = TrainDataset(
            self.id_seq, self.max_len, self.parallel_ag, masked_lm=self.masked_lm
        )
